...
//CÓDIGOS PARA INSERTAR PRODUCTOS DE COCTELERÍA(Carga de datos desde un .JSON local)
const listadoProductos = document.getElementById("listado");

const insertarProductos = () => {
    return fetch('json/cocteleria.json');
}
...
.then(respuesta => {
  listadoProductos.innerHTML = '';
    ...
    contenidoProducto.onclick = () => insertarCanasta(producto);
    // contenidoProducto.addEventListener("click", insertarCanasta(producto));
    listadoProductos.appendChild(contenidoProducto);
  }
})
.catch(error => {
  listadoProductos.innerHTML = error;
})
